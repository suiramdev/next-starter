...
interface PlayerWithTotalPoints {
	userId: string;
	user?: {
		id: string;
		name: string;
...
interface PlayersWithPointsProps {
	players: PlayerWithTotalPoints[];
	isLoading?: boolean;
}

export function PlayersWithPoints({
	players,
	isLoading,
}: PlayersWithPointsProps) {
	if (isLoading) {
		return (
				...
				return (
					<div key={player.userId} className="relative">
						...
						>
							...
							{/* Total points badge at top-right */}
							<Badge
								variant="secondary"
